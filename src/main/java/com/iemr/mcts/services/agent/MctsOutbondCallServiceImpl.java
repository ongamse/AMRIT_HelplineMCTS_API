...
public class MctsOutbondCallServiceImpl implements MctsOutbondCallService
{
	...
	public static OutboundWorklist MapMotherfromObj(Object objects[],Long benRegID,MctsDataReaderDetail readerDetail) {
		return 
				new OutboundWorklist(
						((Number) objects[0]).longValue(), benRegID, (Integer) objects[2], ((Number) objects[3]).longValue(), (String) objects[4], (Timestamp) objects[5],
						(Integer) objects[6], (String) objects[7],((Number) objects[9]).longValue(),readerDetail);
	}
	
	public static OutboundWorklist MapChildfromObj(Object objects[],Long benRegID,Long motherID,ChildValidDataHandler childValidDataHandler) {
		return 
				new OutboundWorklist(
						benRegID, (String) objects[1], (Timestamp) objects[3],
						(Integer) objects[4], (String) objects[2],((Number) objects[11]).longValue(),motherID,((Number) objects[5]).longValue(),childValidDataHandler);
	}
	...
	public String getOutbondCalls(String request) throws IEMRException
	{
		...
		List<Objects[]> motherRecords =mctsOutboundCallRepository.getAgentAllocatedMotherList(mctsOutboundCall.getAllocatedUserID());
		...
		if(regIDList.size()>0)
		{
			ArrayList<MctsOutboundCallDetail> dataByRegIDList= mctsOutboundCallRepository.getLastCallDetailByRegID(regIDList);
		...
		if(obIDList.size()>0)
		{
			ArrayList<MctsOutboundCallDetail> dataByOBIDList= mctsOutboundCallRepository.getLastCallDetailByOBCallID(obIDList);
		...
		List<Objects[]> childRecords = mctsOutboundCallRepository.getAgentAllocatedChildList(mctsOutboundCall.getAllocatedUserID());
		...
		{
			ArrayList<MctsOutboundCallDetail> childDataByRegIDList= mctsOutboundCallRepository.getLastCallDetailByRegID(childRegIDList);
		...
		{
			ArrayList<MctsOutboundCallDetail> childDataByOBIDList= mctsOutboundCallRepository.getLastCallDetailByOBCallID(childOBIDList);
	 ...
	 * allocateCalls(com.iemr.mcts.data.supervisor.AgentCallAllocationDetails) method to move calls from data from valid
	 * data table to outbound call table
	 */
	@Override
	public String allocateCalls(String request) throws IEMRException
	{

		AgentCallAllocationDetail agentCallAllocationDetail =
				inputMapper.gson().fromJson(request, AgentCallAllocationDetail.class);
...
//				mctsOutboundCallRepository.allocateCall(mctsOutboundCall.getObCallID(), users.get(userIndex));
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.iemr.mcts.services.supervisor.AgentCallAllocationServices# getUnallocatedCalls() this method is to get
	 * all mother list details which are not allocated
	 */
	...
	public String getUnallocatedCalls(String request) throws IEMRException
	{
		...
		{
			motherCalls =
					mctsOutboundCallRepository.getMotherUnallocatedCalls(mctsOutboundCall.getProviderServiceMapID());
			childCalls=
					mctsOutboundCallRepository.getChildUnallocatedCalls(mctsOutboundCall.getProviderServiceMapID());
		}
		...
		for (Object[] objects : motherCalls)
		{
			if (objects != null && objects.length > 0)
			{
				...
				unAllocatedCallList.add(new OutboundWorklist(((Number) objects[0]).longValue(), (String) objects[1],(String) objects[2],(Timestamp) objects[3]
						,(String) objects[4],readerDetail));
			}
		}
		...
		for (Object[] objects : childCalls)
		{
			if (objects != null && objects.length > 0)
			{
				...
				unAllocatedCallList.add(new OutboundWorklist(((Number) objects[0]).longValue(), (String) objects[1],(String) objects[2],(Timestamp) objects[3]
						,(String) objects[4],childValidDataHandler));
			}
		}
	 ...
	 * 
	 * @see com.iemr.mcts.services.agent.MctsOutbondCallService#getReallocateCalls( java.lang.String)
	 */
	@Override
	public String getReallocateCalls(String request) throws IEMRException
	{
		...
		for (Integer userID : reallocationDataUtil.getUserIDs())
		{
			...
			if (reallocationDataUtil.getRecordType().equals(MctsConstants.MOTHER))
			{
				...
				if (mctsOutboundCall.getCallDateFrom() != null)
				{
					List<AllocatedCalls> totalCalls = mctsOutboundCallRepository.getUsersMotherAllocatedCalls(
							userID, mctsOutboundCall.getCallDateFrom(), mctsOutboundCall.getCallDateTo());
					Collections.sort(totalCalls, AllocatedCalls.getSortCompoByCallFromDate());
					userMap.put(userID, totalCalls);
				} else
				{

					List<AllocatedCalls> totalCalls =
							mctsOutboundCallRepository.getUsersMotherAllocatedCalls(userID);
					Collections.sort(totalCalls, AllocatedCalls.getSortCompoByCallFromDate());
					userMap.put(userID, totalCalls);
				}
			} else
			{
				if (mctsOutboundCall.getCallDateFrom() != null)
				{
					List<AllocatedCalls> totalCalls = mctsOutboundCallRepository.getUsersChildAllocatedCalls(
							userID, mctsOutboundCall.getCallDateFrom(), mctsOutboundCall.getCallDateTo());
					Collections.sort(totalCalls, AllocatedCalls.getSortCompoByCallFromDate());
					userMap.put(userID, totalCalls);
				} else
				{
					...
					List<AllocatedCalls> totalCalls =
							mctsOutboundCallRepository.getUsersChildAllocatedCalls(userID);
					Collections.sort(totalCalls, AllocatedCalls.getSortCompoByCallFromDate());
					userMap.put(userID, totalCalls);
				}
			}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.iemr.mcts.services.agent.MctsOutbondCallService#moveCallsToBucket( java.lang.String)
	 */
	@Override
...
*/		return "Data moved to bucket successfully";
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.iemr.mcts.services.agent.MctsOutbondCallService#getNextANC_PNC(java.lang. String)
	 */
	@Override
	...
	// private List<MctsOutboundCall> getAgentCallsV0(Integer allocatedUserID)
	// {
	// List<MctsOutboundCall> agentCalls = null;
	...
	// Subquery<MctsOutboundCall> validMotherCall = criteriaQuery.subquery(MctsOutboundCall.class);
	// Root<MctsOutboundCall> validMotherCallRoot = validMotherCall.from(MctsOutboundCall.class);
	// List<Predicate> validMotherCallPredicates = new ArrayList<Predicate>();
	//
	// Subquery<MctsOutboundCall> validOutboundCall = validMotherCall.subquery(MctsOutboundCall.class);
	...
	// validCallPredicates
	// .add(criteriaBuilder.or(criteriaBuilder.isNotNull(validOutboundCallRoot.get("prefferedCallDate")),
	// criteriaBuilder.lessThan(validOutboundCallRoot.get("prefferedCallDate"), currentTime)));
	// validOutboundCall.distinct(true);
	// validOutboundCall.select(validOutboundCallRoot.get("motherID"))
	// .where(validCallPredicates.toArray(new Predicate[]
	// {}));
	// validOutboundCall.groupBy(validOutboundCallRoot.get("motherID"));
	//
	// validMotherCall.select(validMotherCallRoot).where(validMotherCallPredicates.toArray(new Predicate[] {}));
	// MctsOutboundCall validMotherCalls = (MctsOutboundCall) validOutboundCall.as(MctsOutboundCall.class);
	// predicates.add(criteriaBuilder.equal(root.get("obCallID"), validMotherCalls.getObCallID()));
	// criteriaQuery.distinct(true);
	// criteriaQuery.select(root).where(predicates.toArray(new Predicate[] {}));
	// criteriaQuery.orderBy(criteriaBuilder.desc(root.get("outboundCallType")));
	...
	private List<MctsOutboundCall> getAgentCalls(Integer allocatedUserID, Boolean fetchChildRecords)
	{
		...
		{
			Join<MctsOutboundCall, ChildValidDataHandler> beneficiaryRecords =
					root.join("childValidDataHandler", JoinType.INNER);
			criteriaQuery.groupBy(root.get("childID"), root.get("outboundCallType"));
		} else
		{
			Join<MctsOutboundCall, MctsDataReaderDetail> beneficiaryRecords =
					root.join("mctsDataReaderDetail", JoinType.INNER);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.iemr.mcts.services.agent.MctsOutbondCallService#getHighRiskReason( java.lang.String)
	 */
	@Override
	...
	public String getChildWorklist(String request) throws IEMRException
	{
		...
		List<Objects[]> motherRecords =mctsOutboundCallRepository.getAgentAllocatedMotherChildList(mctsOutboundCall.getAllocatedUserID());
